...
class AppConfigStoreRequest extends FormRequest
{
    /**
     ...
     *
     * 约束：
     * 1. configKey 在同一 env+platform 下必须唯一；
     * 2. visibilityMode=window 时必须显式传入 windows；
     * 3. windows 的时间格式校验放宽为 date，兼容多端传值。
     *
    ...
    public function rules()
    {
        return [
            'configKey' => [
                ...
                'max:100',
                'regex:/^[a-zA-Z0-9._-]+$/',
                Rule::unique('app_config', 'config_key')->where(function ($query) {
                    return $query
