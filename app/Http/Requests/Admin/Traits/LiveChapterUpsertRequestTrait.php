 ...
 * 2. 强校验 courseId 必须为直播课（play_type=直播课）；
 * 3. 按 unlockType 触发条件必填，确保解锁规则可追溯。
 */
trait LiveChapterUpsertRequestTrait
{
    /**
     * 直播章节 upsert 公共规则。
     *
     ...
     */
    protected function liveChapterUpsertRules(): array
    {
        return [
            'courseId' => [
                'required',
            ...
            'liveRoomId' => [
                'required',
                'integer',
                Rule::exists('app_live_room', 'room_id')->whereNull('deleted_at'),
            ],
     ...
     */
    protected function liveChapterUpsertMessages(): array
    {
        return [
            'courseId.required' => '请选择直播课程。',
            'courseId.integer' => '课程ID必须是整数。',
            'courseId.exists' => '课程不存在或不是直播课。',
            ...
            'liveRoomId.required' => '请选择直播间。',
            'liveRoomId.integer' => '直播间ID必须是整数。',
            'liveRoomId.exists' => '直播间不存在或已删除。',
