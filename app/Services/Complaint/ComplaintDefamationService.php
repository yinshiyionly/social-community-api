...
class ComplaintDefamationService
{
     ...
     */
    protected function collectAttachmentPaths(ComplaintDefamation $complaint): array
    {
        $attachmentPaths = [];
    ...
    public function audit(int $id, int $reportState): bool
    {
        ...
        // 校验目标状态是否有效
        $allowedStates = [
            ComplaintDefamation::REPORT_STATE_PLATFORM_REJECTED,   // 2-平台驳回
            ComplaintDefamation::REPORT_STATE_PLATFORM_APPROVED,   // 3-平台审核通过
            ComplaintDefamation::REPORT_STATE_OFFICIAL_REVIEWING,  // 4-官方审核中
        ];
        ...
        try {
            ...
            // 记录审核操作日志
            Log::channel('daily')->info('诽谤类投诉审核操作成功', [
                'complaint_id' => $id,
                'old_state' => ComplaintDefamation::REPORT_STATE_PLATFORM_REVIEWING,
                'new_state' => $reportState,
                'new_state_label' => ComplaintDefamation::REPORT_STATE_LABELS[$reportState] ?? '未知',
            ]);
