 ...
 * 诽谤资料管理服务
 *
 * 负责诽谤资料的业务逻辑处理，包括CRUD操作和材料字段URL处理
 */
class MaterialDefamationService
{
    /**
     * 文件上传服务实例
     *
     ...
     *
     * @param int $id 诽谤资料ID
     * @return MaterialDefamation
     * @throws ApiException
     */
    public function getById(int $id): MaterialDefamation
    {
        $item = MaterialDefamation::find($id);
     ...
     * 创建诽谤资料
     *
     * @param array $data 诽谤资料数据
     * @return MaterialDefamation
     */
    public function create(array $data): MaterialDefamation
    {
        return DB::transaction(function () use ($data) {
            // 处理材料字段URL（移除schema和host）
            $data = $this->processMaterialUrlsForStorage($data);

            return MaterialDefamation::create($data);
        });
    }
     ...
     *
     * @param MaterialDefamation $model 诽谤资料模型
     * @return MaterialDefamation 处理后的模型
     */
    public function processMaterialUrlsForDisplay(MaterialDefamation $model): MaterialDefamation
    {
