...
| 删除百家云视频（单个） | DELETE | `/api/admin/video/baijiayun/{videoId}` |
...
| 参数   | 类型   | 必填 | 说明                        |
|------|------|----|---------------------------|
| file | file | 是  | 视频文件，支持 MIME： `video/mp4` |
...
- 说明：
    - 仅支持单个 `videoId` 删除。
    - 旧版逗号分隔路径（如 `123456,123457`）不再支持，路由层会返回 `404`。
    - 逻辑删除（软删除）。
