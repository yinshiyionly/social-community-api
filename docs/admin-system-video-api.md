...
| 删除系统视频（支持批量） | DELETE | `/api/admin/video/system/{videoIds}` |
...
- 说明：`videoId` 由数据库自增生成（`app_video_system.video_id`），创建接口无需传入。
...
```json
{
  ...
  "playUrl": "https://cdn.example.com/video/new.mp4",
  "length": 300,
  "width": 1920,
  "height": 1080
}
...
### 3.6 删除系统视频（支持批量）
- 方法：`DELETE`
- 路径：`/api/admin/video/system/{videoIds}`
- 说明：
  - 支持批量删除，`videoIds` 使用英文逗号分隔，例如：`7101,7102,7103`。
  - 逻辑删除（软删除）。
  - 若视频已被 `admin_video_chapter_content` 引用，接口会拒绝删除。
