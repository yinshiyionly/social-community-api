...
> 后端请按 `postType` 返回对应主字段，避免前端二次猜测数据类型。
...
- `postType = 2`（视频）
  - `videoUrl`：必填（可直接播放的视频地址）。
  - `videoCover`：建议必填（视频封面）；不传时前端回退使用 `coverImage`。
  - `images`：传空数组 `[]`（不要与视频图集混传）。
  - `content`：建议必填（视频文案）。

- `postType = 3`（文章）
  - `title`：建议必填（文章标题）。
  - `content`：建议必填（文章正文/摘要）。
  - `videoUrl`：传空字符串 `""`。
  - `images`：传空数组 `[]`。

- 通用字段（所有 `postType` 都应返回）
  - `id`、`postType`、`author.memberId`、`author.nickname`、`author.avatar`
  - `isLiked`、`isFavorited`、`isFollowed`
  - `likeCount`、`favoriteCount`、`commentCount`、`createTime`
